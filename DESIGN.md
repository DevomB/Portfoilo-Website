...
description: Backend engineer portfolio. Dual-audience design — readable by anyone, appreciated by developers.
colors:
  system-teal: "#276787"
  system-teal-dim: "#1c4d68"
  system-teal-pale: "#c4d8e8"
  field-cream: "#faf5e6"
  surface: "#f3ead2"
...
components:
  button-primary:
    backgroundColor: "{colors.system-teal}"
    textColor: "{colors.surface-elevated}"
    rounded: "{rounded.button}"
    padding: "0.5rem 1rem"
  button-primary-hover:
    backgroundColor: "{colors.system-teal-dim}"
    textColor: "{colors.surface-elevated}"
  button-ghost:
...
- Flat surfaces at rest — depth comes from tonal shift and border color, never shadows
...
### Primary
- **System Teal** (`#276787`): The structural ink. Used on: the hero H1, section `// labels`, link hover states, interactive focus indicators, the accent dot in the nav logo. Appears where meaning is being anchored, not where decoration is added.
...
### Neutral
- **Field Cream** (`#faf5e6`): The page canvas. All content renders on this surface.
- **Warm Surface** (`#f3ead2`): Subtly darker cream. Used for the server log panel background and as a secondary surface.
- **Surface Elevated** (`#fdf9ea`): Near-white cream. The lightest neutral layer, used for elevated or focused surfaces.
...
### Named Rules
**The Two-Color Rule.** The palette is System Teal and Field Cream. No other hue is introduced at any point. When tonal variation is needed, step along the existing hues toward white or black. A third color is not a refinement — it is a violation.
...
The pairing has deliberate role division: Bricolage Grotesque handles everything human-facing — headings, body copy, names, CTAs; Geist Mono handles everything machine-facing — section labels, dates, stack items, code, terminal text, index numbers. The typeface switch is the signal. When you see Geist Mono, you are reading data.
...
**The Mono = Data Rule.** If an element represents data — a date, a stack item, a code snippet, a terminal line, a row index, a section identifier — it uses Geist Mono. If it represents natural language — a name, a job title, a bio sentence — it uses Bricolage Grotesque.

## 4. Elevation

This system is flat by default. Depth is conveyed through tonal variation between Field Cream surfaces and Border Warm dividers — not through shadows.
...
Hover interaction on linked rows uses `opacity: 0.7` — a signal of interactivity without lifting the surface. The interaction is reticent: the element doesn't come toward you, it acknowledges you.
...
- **Badge (optional):** Geist Mono, `0.65rem`, System Teal, flush right, for "live demo ↗" or "site ↗"
...
- **Mobile:** Hamburger toggle opens a drawer from the navbar — same cream background, same link treatment.
...
- **Log lines:** Geist Mono, `0.75rem`, Ink Muted. Final "ready ✓" line: System Teal.
...
- **Positioning:** Absolute, edges of the hero viewport, slight rotation (±1.5deg). `user-select: none`, `pointer-events: none`.
...
- **Do** let the SQL ghost text and server log be the developer-layer easter eggs. They are intentionally subtle — 4.5% opacity on the ghost text is not a bug.
- **Do** maintain the two-typeface rule across every new component: Bricolage for human content, Geist Mono for machine content.

### Don't:
- **Don't** use dark mode, neon accents, glow effects, Matrix aesthetics, or particle backgrounds. This is an explicit anti-reference from PRODUCT.md. The site is light-on-cream by design, always.
